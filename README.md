...
**[printrow.lastra.us](https://printrow.lastra.us)** — a label designer that prints against CSV data over Web Bluetooth. No drivers, no installs, no print server: design a label in the browser, bind `{{variables}}` to CSV columns, and batch-print to a Y50P thermal printer directly from the page.
...
Verified on a **Y50P** (FlashLabel), 50 × 30 mm stock at 8 dots/mm. Other media heights are safe — the protocol never transmits height, so the printer simply takes rows until the raster ends. Widths other than 50 mm follow the captured frame format but have not been tested on real stock.
